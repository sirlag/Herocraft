...
export const actions: Actions = {
	default: async (event) => {
		...
		let importFormat = includedDecks.map(
			({ name, trait1, trait2, deck: rawDeck, specialization }) => {
		...
		let uniqueCards = uniqueDecks.map((it, deckIndex) => {
			return {
				...
				traits: [importFormat[deckIndex].trait1, importFormat[deckIndex].trait2],
