<script lang="ts">
  ...
  // New features: layout, variants, multi-face + image URLs
  type CardLayout = 'NORMAL' | 'TRANSFORM' | 'TOKEN';
  let layout: CardLayout = $state('NORMAL');
  let variantsText = $state(''); // comma-separated UUIDs

  // Front face optional overrides + image URIs
  let frontName = $state('');
  ...
  $effect(() => {
    // If there's a back face, default layout to TRANSFORM; otherwise NORMAL
    layout = hasBackFace ? 'TRANSFORM' : layout;
  });
  ...
  async function save() {
    ...
    try {
      ...
      window.location.href = `/admin/cards/${created.id}`;
    } catch (e) {
...
<div class="container mx-auto p-4 space-y-4">
  ...
  <form class="grid gap-3 max-w-3xl" on:submit|preventDefault={save}>
    <label class="grid gap-1">
      <span>Name</span>
      <input class="input input-bordered p-2 border rounded" bind:value={name} required />
    </label>
    <label class="grid gap-1">
      <span>Artist</span>
      <input class="input input-bordered p-2 border rounded" bind:value={artist} required />
    </label>
    <label class="grid gap-1">
      <span>Season</span>
      <input class="input input-bordered p-2 border rounded" bind:value={season} required />
    </label>
    ...
    <!-- Live rules preview rendered like the normal card page -->
    ...
    <div class="grid grid-cols-2 gap-3">
      ...
      <label class="grid gap-1">
        ...
        <input class="p-2 border rounded" bind:value={variantsText} placeholder="uuid1, uuid2, ..." />
      </label>
    </div>
    ...
    <fieldset class="border rounded p-3 space-y-3">
      ...
      <div class="grid grid-cols-2 gap-3">
        ...
        <label class="col-span-2 grid gap-1">
          ...
          <textarea class="p-2 border rounded" bind:value={frontRules} rows="3" placeholder="defaults to Rules Text"></textarea>
        </label>
        <label class="col-span-2 grid gap-1">
          <span>Flavor Text (override)</span>
          <textarea class="p-2 border rounded" bind:value={frontFlavor} rows="2" placeholder="defaults to Flavor Text"></textarea>
        </label>
        <label class="grid gap-1">
    ...
    <div class="flex gap-2">
      ...
      <a href="/admin/cards" class="underline self-center">Cancel</a>
    </div>
  </form>
</div>
