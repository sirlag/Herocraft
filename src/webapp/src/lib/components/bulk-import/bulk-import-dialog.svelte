	...
	const viewDecks = (file: File) => {
		if (file) {
			...
			reader.addEventListener('load', () => {
				listOfDecks =
					typeof reader.result === 'string' ? (listOfDecks = JSON.parse(reader.result)) : undefined;
			});
	...
	<Dialog.Content>
		...
		>
			...
			<Form.Fieldset {form} name="defaultVisibility">
				...
				<RadioGroup.Root bind:value={$formData.defaultVisibility} class="flex flex-row py-4">
